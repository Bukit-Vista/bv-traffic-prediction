...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await readCatchmentFlows({ origin, destination, minScore, limit });
    return internalCatchmentJson(request, result.meta, {
      ...
      flows: result.flows
    }, { origin: result.originCatchmentKey, destination: result.destinationCatchmentKey, minScore: result.minScore, limit });
  } catch (error) {
