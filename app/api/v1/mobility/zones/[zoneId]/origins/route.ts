...
export async function GET(request: Request, context: { params: Promise<{ zoneId: string }> }) {
  try {
    ...
    const result = await getMobilityFlows({ bbox: [114.34,-8.9,115.78,-8.03], at: url.searchParams.get("at") ?? "latest", limit: Math.min(100, Number(url.searchParams.get("limit") ?? 10)), minScore: 0, destinationZoneId: zoneId });
